...
function gen_otp(mobile, res, successCallback, errorCallback) {
    var otp = mobile == "8010265036" ? 123456 : Math.floor(Math.random() * (999999 - 111111 + 1)) + 111111;
    var where = {};
    where["otp"] = otp;
    Users.findOne(where).then((response) => {
        if (response == null) {
...
module.exports = {
    ...
    product_search: function (req, res) {
        ...
        MasterCategory.findOne(where).then((master_response) => {
            ...
            if (master_response) {
                where["master_category"] = { $in: [mongoose.Types.ObjectId(master_response._id)] };
            }
            ...
            if (req.query.sub_category && req.query.sub_category != "") {
                ...
                for (let i = 0; i < req.query.sub_category.split(",").length; i++) {
                    sub_category.push(mongoose.Types.ObjectId(req.query.sub_category.split(",")[i]));
                }
            ...
            if (req.query.category && req.query.category != "") {
                ...
                for (let i = 0; i < req.query.category.split(",").length; i++) {
                    category.push(mongoose.Types.ObjectId(req.query.category.split(",")[i]));
                }
            ...
            if (req.query.option && req.query.option != "") {
                ...
                for (let i = 0; i < options.length; i++) {
                    q.push({
                        "options.value": { $regex: ".*" + options[i].trim() + "*.", $options: "i" },
                    });
                }
                where["$or"] = q;
            }
                    ...
                    Coupon.findOne(where).then((response_coupon) => {
                        for (var i = 0; i < response.length; i++) {
                            ...
                            if (response[i].product != null) {
                                ...
                                if (response_coupon && response_coupon.brand && response_coupon && response[i].product.brand && !response_coupon.brand.includes(response[i].product.brand._id)) {
                                    res.status(200).send({
                                        status: "error",
                                        message: "Coupon is not applicable",
                                        result: [],
                                ...
                                if (response_coupon && response_coupon.category && !response_coupon.category.includes(response[i].product.category)) {
                                    res.status(200).send({
                                        status: "error",
                                        message: "Coupon is not applicable.",
                                        result: [],
                ...
                Coupon.findOne(where).then((coupon_response) => {
                    ...
                    Cart.find(where).then((cart_response) => {
                        ...
                        for (let i = 0; i < cart_data.length; i++) {
                            ...
                            cart_data[i].option.map((item2) => (final_price = final_price + Number(item2.price)));
                        ...
                        var checkoutData = new Checkout({
                            ...
                            note: req.body.note,
                            tax: req.body.total_tax && req.body.total_tax != "" ? req.body.total_tax : 0,
                                ...
                                checkoutData.save().then((response) => {
                                    if (req.body.billing.same == "false") {
                                        var userAddress = new Address({
                                            ...
                                            contact_name: req.body.billing.address.fname + " " + req.body.billing.address.lname,
                    ...
                    Checkout.findOne(where).then((checkout_response) => {
                        ...
                        for (let i = 0; i < checkout_response.products.length; i++) {
                            ...
                            Products.findOne(where).then((product_response) => {
                                if (checkout_response.products[i].option.length == 0) {
                                    var newstock = Number(product_response.stock_qty) - Number(checkout_response.products[i].quantity);
                                    ...
                                    for (let i = 0; i < product_response.options.length; i++) {
                                        for (let j = 0; j < checkout_response.products[i].option.length; j++) {
                                            if (product_response.options[i].name == product_response.options[i].value.key) {
                                                var index = 0;
                                                for (let k = 0; k < product_response.options[i].value.value.split(",").length; k++) {
                                                    if (product_response.options[i].value.value.split(",")[k].includes(product_response.options[i].value.value)) {
                                                        index = k;
                                                    }
                                                }
                                                var selected_option = product_response.options[i].value.split(",")[index].split(":");

                                                console.log("selected_option", selected_option);

                                                var new_option = selected_option[0] + ":" + selected_option[1] + ":" + selected_option[2] + ":" + Number(selected_option[3] - Number(checkout_response.products[i].quantity));

                                                var option = product_response.options[i];
                                                var new_option2 = option.value.split(",");
                                                new_option2.splice(new_option2.indexOf(option.value.split(",")[index]), 1, new_option);
                                ...
                                var sale_count = product_response.sale_count ? product_response.sale_count : 0;
                            ...
                            if (checkout_response.products[i].option[0] && checkout_response.products[i].option[0].value.includes("#")) {
                            ...
                            if (checkout_response.products[i].option[1] && checkout_response.products[i].option[1].value.includes("#")) {
                        ...
                        var data = JSON.stringify({
                            ...
                            fields: {
                                ...
                                date: moment(moment(checkout_response.created_date, "DD-MM-YYYY")).format("DD-MM-YYYY"),
                                memo: "ffff",
    ...
    header_search: function (req, res) {
        MasterCategory.aggregate([
            ...
            var arr = [];
            let uniqueObjArray = [...new Map(category_response.map((item) => [item["name"], item])).values()];
        ...
        if (cart) {
            ...
            Products.findOne(where).then((response) => {
                if (response.selling_price) {
                    ...
                    try {
                        if (response.selling_price.value && response.selling_price.value != NaN) {
    ...
    cart_list: function (req, res) {
        if ((!req.body.uniqueid && !req.body.id) || (req.body.uniqueid == "" && req.body.id == "")) {
            res.status(200).send({
                status: "success",
                result: [],
            });
    ...
    add_review: function (req, res) {
        Checkout.find({
            user: req.body.user,
            products: { $elemMatch: { product: mongoose.Types.ObjectId(req.body.product) } },
    ...
    get_pg_response: function (req, res) {
        ...
        axios(config)
            .then(function (response) {
                var where = {};
                where["orderid"] = req.body.orderid;
                Checkout.findOneAndUpdate(where, { transactionid: response.data.Transaction.TransactionID }, { new: true })
                    .exec()
                    .then((response3) => {
                        res.status(200).send({
                            status: "success",
        ...
        Checkout.findOne(where).then((response) => {
            ...
            axios(config)
                .then(function (response2) {
                    if (response2.data.Transaction.ResponseClassDescription == "Success") {
                        var where = {};
                        where["orderid"] = req.body.order;
                        Checkout.findOneAndUpdate(where, { status: "paid", gateway: "ONLINE" }, { new: true })
                            .exec()
                            .then((response3) => {
                                res.status(200).send({
                                    status: "success",
                    ...
                    if (response2.data.Transaction.ResponseClassDescription == "Success") {
                        ...
                        Checkout.findOne(where)
                            ...
                            .then((response) => {
                                ...
                                var where = {};
                                where["orderid"] = req.body.order;
                                Checkout.findOneAndUpdate(where, { status: "paid", gift_code: gift_code }, { new: true })
                                    .exec()
                                    .then((response3) => {
                                        let transporter = nodemailer.createTransport({
                                            host: SMTP_HOST,
                                        ...
                                        transporter
                                            .sendMail({
                                                ...
                                                html: email_html("Hi,<br/> " + response.gift_email + " sent gift card with you. <br/> Code: " + gift_code + " <br/> Amount: " + response.finalprice + " <br/>   "), // html body
                                            })
                                            .then((response3) => {
                        ...
                        Wallet.aggregate([
                            ...
                            Wallet.aggregate([
                                ...
                                res.status(200).send({
                                    ...
                                    result: response,
                                    totalCount: count,
                                    balance: Number(sum_in.length > 0 ? sum_in[0].totalValue : 0) - Number(sum_out.length > 0 ? sum_out[0].totalValue : 0),
                                });
                            });
                        });
                    });
    ...
    deactivate_account: function (req, res) {
        ...
        Users.findOneAndUpdate(where, { login_active: 0, deactivation_reason: req.body.deactivation_reason }, { new: true })
            .exec()
            .then((response) => {
                res.status(200).send({
                    status: "success",
    ...
    mobile_home: async function (req, res) {
        ...
        for (let i = 0; i < master.length; i++) {
            var p = await MobileCategory.find({ master_category: master[i]._id }, null, {
                limit: 2,
            })
                .populate("product")
                .populate("brand");
        ...
        var featured = await Products.aggregate([{ $match: { featured: 1, active: 1, deleted: 0 } }, { $sample: { size: 3 } }]);
    ...
    mobile_category: async function (req, res) {
        ...
        var section = await MobileCategory.find(where).populate("product").populate("master_category");
    ...
    sync_cart: async function (req, res) {
        Cart.updateMany({ uniqueid: req.body.uniqueid }, { user: req.body.id }, function (err, docs) {
            if (err) {
                console.log(err);
            }
                ...
                Users.findOne(where)
                    .then((response) => {
                        ...
                        if (response.deleted == 1) {
                            res.status(200).send({
                                status: "error",
                                message: "Your account has been suspended, please contact admin to proceed further",
                            });
                            return;
                        }
    ...
    login_google: async function (req, res) {
        var GOOGLE_CLIENT_ID = "270690778965-fvfnfmf1kluddnj8ud62msprnn855euu.apps.googleusercontent.com";
                    ...
                    MasterCategory.find(where).then((master_response) => {
                        ...
                        for (let i = 0; i < master_response.length; i++) {
                            ...
                            for (let j = 0; j < response.length; j++) {
                                if (response[j].master_category.includes(master_response[i]._id)) {
                ...
                if (product_response.options.length == 0) {
                    ...
                    for (let i = 0; i < product_response.options.length; i++) {
                        if (product_response.options[i].name == req.body.option) {
                            var index = 0;
                            for (let k = 0; k < product_response.options[i].value.split(",").length; k++) {
                                if (product_response.options[i].value.split(",")[k].includes(req.body.value)) {
                                    index = k;
                                }
                            }
                            var selected_option = product_response.options[i].value.split(",")[index].split(":");

                            var new_option = selected_option[0] + ":" + selected_option[1] + ":" + selected_option[2] + ":" + Number(req.body.qty);
